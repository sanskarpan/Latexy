...
function SortableRow({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`flex items-center gap-2 rounded-[var(--radius-md)] px-2 py-2 transition-colors ${
        isDragging ? 'bg-surface-2' : 'hover:bg-surface-2'
      }`}
    >
      <button
        {...attributes}
        {...listeners}
        className="shrink-0 cursor-grab touch-none text-fg-3 hover:text-fg-2 active:cursor-grabbing"
        aria-label="Drag to reorder"
      >
        <GripVertical size={14} />
      </button>
      <span className="flex-1 truncate text-[12px] text-fg">{label}</span>
      {moved && (
        <span className="shrink-0 rounded bg-accent-soft px-1.5 py-0.5 text-[10px] text-accent-strong border border-accent">
          moved
        </span>
      )}
    </div>
...
export default function SectionReorderPanel({
  ...
  return (
    ...
    >
      <div className="w-full max-w-xl rounded-[var(--radius-lg)] border border-line bg-surface shadow-[var(--shadow-2)]">
        {/* ── Header ── */}
        <div className="flex items-center justify-between border-b border-line px-4 py-3">
          <div className="flex items-center gap-2">
            <div className="flex h-6 w-6 items-center justify-center rounded-[var(--radius-md)] bg-accent-soft">
              <Layers size={13} className="text-accent-strong" />
            </div>
            <h2 className="text-sm font-semibold text-fg">AI Section Reordering</h2>
          </div>
          <button
            onClick={onClose}
            aria-label="Close"
        ...
        <div className="space-y-4 p-4">
          ...
          <div className="space-y-3">
            {/* Career stage */}
            <div>
              <label className="mb-1 block text-[11px] font-medium text-fg-3">
                Career stage (optional)
              </label>
              <select
                value={careerStage}
                onChange={(e) => setCareerStage(e.target.value as CareerStage)}
                className="w-full rounded-[var(--radius-md)] border border-line bg-bg px-3 py-2 text-[12px] text-fg focus:border-accent focus:outline-none"
              >
            ...
            <div className="rounded-[var(--radius-md)] border border-line bg-bg">
              ...
              >
                <span>Job description <span className="normal-case text-fg-3">(optional — improves suggestion)</span></span>
              ...
              {jdOpen && (
                <div className="px-3 pb-3">
                  <textarea
                    ...
                    className="w-full resize-none rounded-[var(--radius-md)] border border-line bg-bg px-3 py-2 text-[12px] text-fg placeholder-fg-3 focus:border-accent focus:outline-none"
                  />
                  <p className="mt-0.5 text-right text-[10px] text-fg-3">
          ...
          <button
            ...
            disabled={loading}
            className="flex w-full items-center justify-center gap-2 rounded-[var(--radius-md)] border border-line-2 bg-surface-2 py-2 text-xs font-semibold text-fg transition hover:bg-surface-2 disabled:opacity-50"
          >
            {loading ? (
              <><Loader2 size={12} className="animate-spin" /> Thinking…</>
            ) : (
              <><Sparkles size={12} className="text-accent-strong" /> Suggest Section Order</>
            )}
          </button>
          ...
          {result && result.current_order.length > 0 && (
            <div className="space-y-3">
              {/* Rationale */}
              <div className="rounded-[var(--radius-md)] border border-line bg-accent-soft px-3 py-2">
                <p className="text-[11px] leading-relaxed text-fg-2">{result.rationale}</p>
                {result.cached && (
                  <span className="mt-1 inline-block rounded bg-surface-2 px-1.5 py-0.5 text-[10px] text-fg-3">
                    cached
                  </span>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <div className="space-y-0.5 rounded-[var(--radius-md)] border border-line bg-bg p-2">
                    {result.current_order.map((name, i) => (
                      <div
                        key={name}
                        className="flex items-center gap-1.5 rounded px-2 py-1.5"
                      >
                        <span className="w-4 shrink-0 text-[10px] text-fg-3">{i + 1}</span>
                        <span className="truncate text-[12px] text-fg-2">{name}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  <p className="mb-1.5 text-[10px] font-semibold uppercase tracking-wider text-fg-3">
                    AI Suggestion <span className="normal-case text-fg-3">(drag to adjust)</span>
                  </p>
                  <div className="rounded-[var(--radius-md)] border border-line bg-bg p-1">
                    <DndContext
                      sensors={sensors}
              ...
              <div className="rounded-[var(--radius-md)] border border-line bg-bg">
                <button
                  ...
                  className="flex w-full items-center justify-between px-3 py-2 text-[11px] font-semibold uppercase tracking-wider text-fg-3"
                >
                  <span>Diff preview <span className="normal-case text-fg-3">(first 12 lines of each)</span></span>
                  {diffOpen ? <ChevronDown size={12} /> : <ChevronRight size={12} />}
                </button>
                {diffOpen && (
                  <div className="grid grid-cols-2 gap-0 divide-x divide-line px-0 pb-3 pt-0">
                    <div className="px-3">
                      <p className="mb-1 text-[10px] text-fg-3">Current</p>
                      <pre className="overflow-x-auto whitespace-pre-wrap break-all font-mono text-[10px] leading-relaxed text-fg-3">
                    ...
                    <div className="px-3">
                      <p className="mb-1 text-[10px] text-fg-3">Reordered</p>
                      <pre className="overflow-x-auto whitespace-pre-wrap break-all font-mono text-[10px] leading-relaxed text-fg-3">
                        {userOrder
                          .map((n) => `\\section{${n}}`)
                          .join('\n...\n')
              ...
              {JSON.stringify(userOrder) === JSON.stringify(result.current_order) && (
                <div className="flex items-start gap-2 rounded-[var(--radius-md)] border border-warn/20 bg-warn/[0.04] px-3 py-2">
              ...
              <button
                ...
                className="flex w-full items-center justify-center gap-2 rounded-[var(--radius-md)] bg-accent py-2 text-xs font-semibold text-accent-fg transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-40"
              >
                {loading ? (
