    ...
    <article className={`surface-card relative p-5 ${isPopular ? 'edge-highlight bg-orange-300/5' : ''}`}>
      <div className="absolute right-4 top-4 flex gap-2">
        {plan.discount_percent ? (
          <span className="rounded-full border border-emerald-300/30 bg-emerald-300/10 px-2 py-1 text-[10px] uppercase tracking-wider text-emerald-100">
            Save {plan.discount_percent}%
          </span>
        ) : null}
        {isPopular && (
          <span className="rounded-full border border-orange-200/30 bg-orange-300/20 px-2 py-1 text-[10px] uppercase tracking-wider text-orange-100">
            Recommended
          </span>
        )}
      </div>
      ...
      <div className="mb-5">
        ...
        <p className="mt-2 text-3xl font-semibold text-white">{formatPrice(plan.price)}</p>
        <p className="text-sm text-slate-400">{plan.price > 0 ? `per ${plan.interval}` : 'No payment required'}</p>
        {plan.monthly_equivalent_price ? (
          <p className="mt-1 text-xs text-emerald-200">₹{(plan.monthly_equivalent_price / 100).toFixed(0)}/month effective</p>
        ) : null}
        {plan.requires_student_verification ? (
          <p className="mt-1 text-xs text-sky-200">Requires student email verification</p>
        ) : null}
        {plan.max_seats ? (
          <p className="mt-1 text-xs text-amber-200">Includes {plan.max_seats} seats</p>
        ) : null}
      </div>
